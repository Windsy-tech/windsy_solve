...
class NCActionsRepository {
  ...
  NCActionsRepository({required FirebaseFirestore firestore}) : _firestore = firestore;
  ...
  FutureVoid addActionToNC(String companyId, NCActionsModel ncActionsModel) async {
    try {
      return right(_companies.doc(companyId).collection('ncActions').add(ncActionsModel.toMap()));
